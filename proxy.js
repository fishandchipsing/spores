...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && req.url === '/squirrel') {
    const file = path.join(__dirname, 'squirrel.html');
    if (fs.existsSync(file)) {
      res.writeHead(200, { 'Content-Type': 'text/html' });
      fs.createReadStream(file).pipe(res);
    } else {
      res.writeHead(404); res.end('squirrel.html not found');
    }
    return;
  }
  ...
  if (req.method === 'GET' && req.url === '/murmur') {
    const file = path.join(__dirname, 'murmur-radio-01.html');
    if (fs.existsSync(file)) {
      res.writeHead(200, { 'Content-Type': 'text/html' });
      fs.createReadStream(file).pipe(res);
    } else {
      res.writeHead(404); res.end('murmur-radio-01.html not found');
    }
    return;
  }
  ...
  if (req.method === 'GET' && req.url === '/ecosystem') {
    const file = path.join(__dirname, 'ecosystem-seq.html');
    if (fs.existsSync(file)) {
      res.writeHead(200, { 'Content-Type': 'text/html' });
      fs.createReadStream(file).pipe(res);
    } else {
      res.writeHead(404); res.end('ecosystem-seq.html not found');
    }
    return;
  }
  ...
  if (req.method === 'GET' && req.url === '/murmur2') {
    ...
    if (fs.existsSync(file)) {
      ...
      res.writeHead(404); res.end('murmur-radio-02.html not found');
    }
    return;
  }
